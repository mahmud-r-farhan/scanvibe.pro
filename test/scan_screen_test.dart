...
void main() {
  ...
  testWidgets('renders initial scan screen correctly', (tester) async {
    ...
    expect(find.text('Choose a source to scan and extract text.'), findsOneWidget);
  ...
  testWidgets('shows warning and disables camera button when camera unavailable', (tester) async {
    final fakeCapture = _FakeImageCaptureService(
    ...
    await state.createDocumentFromCamera(); // This is a general exception. To set isCameraAvailable = false, we need PlatformException.
    ...
    expect(find.text('Camera is unavailable. Try importing from your gallery instead.'), findsOneWidget);
...
class _FakeLocalizationsDelegate extends LocalizationsDelegate<ScanVibeLocalizations> {
  ...
  @override
  bool shouldReload(covariant LocalizationsDelegate<ScanVibeLocalizations> old) => false;
}
